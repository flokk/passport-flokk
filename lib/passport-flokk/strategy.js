...
function Strategy(options, verify) {
  options = options || {};
  options.authorizationURL = options.authorizationURL || 'https://www.theflokk.com/login/oauth/authorize';
  options.tokenURL = options.tokenURL || 'https://www.theflokk.com/login/oauth/access_token';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  this._oauth2.get('https://api.theflokk.com/user', accessToken, function (err, body, res) {
    ...
    try {
      ...
      profile.profileUrl = json._links.account.href;
      profile.emails = [];

      if (json._links.email) {
        json._links.email.forEach(function(email) {
          profile.emails.push({ value: email.href });
        });
      }
